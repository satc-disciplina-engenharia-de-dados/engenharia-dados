...
dag = DAG(
    ...
    description='Code to move data from Postgres to Minio in the Silver layer',
...
def clean_and_process_tables(**kwargs):
    '''Process each table, clean, and upload to Minio in the Silver layer'''
    ...
    for table in tables:
        ...
        cleaned_df = clean_data(df)  # Aplica limpeza e transformação na camada "silver"
        csv_file_path = f'/tmp/{table}/{table}_{now}_silver.csv'
        ...
        for data_file in data_files:
            if not data_file.endswith('.csv') and not data_file.endswith('.json'):
                insert_data_on_minio(minio_client, 'silver', os.path.basename(data_file), data_file)  # Envia para o Minio na camada "silver"
